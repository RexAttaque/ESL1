...
bool IMU_BNO055::init()
{
  ...
  if(status)
  {
    d_println(debugLevel::INFO, "      --->Standby for BIT...", "init()");
    ...
    d_println(debugLevel::INFO, "        ---->System Status : " + String(sys_status) + "\r\n        ---->Self Test Results : " + String(self_test) + "\r\n        ---->Error codes : " + String(self_test));

    if(sys_status == 5 && self_test == 15 && error == 0)
    {
      d_println(debugLevel::INFO,"      --->BNO055 Axis Remap...");
      IMU.setAxisRemap(_remapcode);
      IMU.setAxisSign(_remapsign);

      if(goIdle())
      {
        d_println(debugLevel::INFO,"      --->BNO055 Suspended until calibration");
      }
...
bool IMU_BNO055::subCalibrate(uint8_t expectedResult)
{
  ...
  {
    d_println(debugLevel::INFO,"      --->subSensor calibrated, letting it settle...");
    delay(BNO055_const::calibrationSettleTime);
    return true;
  }
  else
  {
    d_println(debugLevel::INFO,"      --->Attempts at calibrating subSensor failed");
    return false;
  }
}
...
bool IMU_BNO055::calibrate()
{
  ...
  {
    d_println(debugLevel::INFO,"Attempt " + String(attempts+1) + "/" + String(BNO055_const::maxCalibrationAttempts), "calibrate()");

    //Gyroscope calibration 0x30
    d_println(debugLevel::INFO,"\n      --->BNO055 Gyroscope subSensor calibration");
    d_println(debugLevel::INFO,"      --->Instructions :");
    d_println(debugLevel::INFO,"        ---->Stand the avionics bay UP RIGHT");
    d_println(debugLevel::INFO,"        ---->Wait for calibration...");
    ...
    d_println(debugLevel::INFO,"      --->Instructions :");
    d_println(debugLevel::INFO,"        ---->Picture the axis system of the BN055 in the UP RIGHT (+z) Avionics bay position (x and y interchangeable).");
    d_println(debugLevel::INFO,"        ---->SLOWLY from that position, rotate the avionics bay to align it with :");
    d_println(debugLevel::INFO,"          ----->The x axis, put down and hold 5s");
    d_println(debugLevel::INFO,"          ----->The -z axis, put down and hold 5s");
    d_println(debugLevel::INFO,"          ----->The -y axis, put down and hold 5s");
    d_println(debugLevel::INFO,"          ----->The -x axis, put down and hold 5s");
    d_println(debugLevel::INFO,"          ----->The y axis, put down and hold 5s");
    d_println(debugLevel::INFO,"          ----->Then back to the UP RIGHT (+z) position, put down and wait 10s");
    d_println(debugLevel::INFO,"        ---->Repeat these steps until calibrated...");
    ...
    d_println(debugLevel::INFO,"      --->Instructions :");
    d_println(debugLevel::INFO,"        ---->Pick up the avionics bay with both hands :");
    d_println(debugLevel::INFO,"          ----->Draw two \"eight\" figures at a time then place the Avionics bay UP RIGHT for 5 seconds");
    d_println(debugLevel::INFO,"        ---->Repeat these steps until calibrated...");
  ...
  {
    d_println(debugLevel::INFO,"      --->BNO055 System calibrated, waiting for it to settle...");
    delay(BNO055_const::calibrationSettleTime);
    return true;
  }
  else
  {
    d_println(debugLevel::INFO,"      --->Attempts at calibrating BNO055 failed");
    return false;
  }
}
